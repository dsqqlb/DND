...
| `core/state.js` | 122 | localStorage 读写、状态初始化、存档对账（按配置对齐法术位/引导神力）、派生值计算 |
| `core/render.js` | 605 | 静态卡面、HP 血条、法术面板（环阶配置驱动）、增益芯片（轮数计时）、死亡豁免、力竭等 |
| `core/dialogs.js` | 531 | 对话框系统、法术选择器（动态环阶标签 + 中英搜索）、增益选择器、升环环阶选择框 |
| `features/rest.js` | 247 | 长休/短休（法术位按配置恢复）、HP 编辑、死亡豁免、力竭、专注、角色备份导出/导入 |
| `features/initiative.js` | 414 | 先攻追踪器（新增/删除/排序/当前回合高亮，回合推进派发事件） |
| `features/bag.js` | 230 | 背包物品数量调整、记事本持久化、货币管理、负重与搬运上限 |
| `features/log.js` | 597 | 冒险日志 + 跑团计时器：会话文件夹、自动记录、筛选、重命名、战报小结、独立导入/导出 |
| `main.js` | 94 | 页面入口：首次渲染所有面板、标签页路由、面板折叠、幸运骰交互 |
| **总计** | **4020** | 数据、配置与逻辑完全分离：日常升级只碰 `config/` 与 `data/` |
