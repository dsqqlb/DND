...
| 文件 | 行数 | 职责 |
|:---|---:|:---|
| `config/character.js` | 76 | 角色配置：属性、等级、法术位、领域法术、技能熟练 |
| `data/spells.js` | 914 | 法术数据库（含戏法、1-3 环自选法术、领域法术） |
| `data/items.js` | 102 | 物品数据库（武器、护甲、装备、消耗品） |
| `data/buffs.js` | 52 | 状态/增益库（9 种增益、14 种状态、4 种战术） |
| `core/state.js` | 85 | localStorage 读写、全局状态初始化、派生值计算（熟练加值、法术 DC、先攻等） |
| `core/render.js` | 371 | 静态卡面渲染、HP 血条、法术面板、增益芯片、死亡豁免、力竭计数等 |
| `core/dialogs.js` | 407 | 自定义对话框系统、法术选择器、增益选择器（模态框 + 搜索 + 分类筛选） |
| `features/rest.js` | 236 | 长休/短休逻辑、HP 编辑、死亡豁免、力竭、专注管理 |
| `features/initiative.js` | 412 | 先攻追踪器（新增/删除/排序/当前回合高亮） |
| `features/bag.js` | 199 | 背包物品数量调整、记事本持久化、货币管理、数据导出/导入 |
| `main.js` | 91 | 页面入口：首次渲染所有面板、标签页路由、面板折叠、幸运骰交互 |
| **总计** | **2945** | 原 `character_sheet.js` 2500 行拆分为 11 个模块，数据与配置完全分离 |
