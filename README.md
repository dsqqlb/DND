...
| `core/state.js` | 121 | localStorage 读写、状态初始化、存档对账（按配置对齐法术位/引导神力）、派生值计算 |
| `core/render.js` | 526 | 静态卡面、HP 血条、法术面板（环阶完全由配置驱动）、增益芯片、死亡豁免、力竭等 |
| `core/dialogs.js` | 499 | 对话框系统、法术选择器（环阶标签动态生成）、增益选择器、升环环阶选择框 |
| `features/rest.js` | 245 | 长休/短休（法术位按配置恢复）、HP 编辑、死亡豁免、力竭、专注、角色备份导出/导入 |
| `features/initiative.js` | 413 | 先攻追踪器（新增/删除/排序/当前回合高亮） |
| `features/bag.js` | 224 | 背包物品数量调整、记事本持久化、货币管理 |
| `features/log.js` | 533 | 冒险日志 + 跑团计时器：会话文件夹、自动记录、分类筛选、重命名、独立导入/导出 |
| `main.js` | 94 | 页面入口：首次渲染所有面板、标签页路由、面板折叠、幸运骰交互 |
| **总计** | **3835** | 数据、配置与逻辑完全分离：日常升级只碰 `config/` 与 `data/` |
